...
from training.policy_v1 import ACTPolicy, CNNMLPPolicy, EMBPolicy
...
class EpisodicDataset(torch.utils.data.Dataset):
    def __init__(self, episode_ids, dataset_dir, camera_names, norm_stats, task_id):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            qpos = root['/observations/qpos'][start_ts] # (5,)
            # qvel = root['/observations/qvel'][start_ts]
            image_dict = dict()
            for cam_name in self.camera_names:
                image_dict[cam_name] = root[f'/observations/images/{cam_name}'][start_ts]   # (h, w, 3)
        ...
        padded_action[:action_len] = action     # shift actions to the beginning (by Hao)
        ...
        all_cam_images = np.stack(all_cam_images, axis=0)   # (1, h, w, 3)

        # construct observations
        image_data = torch.from_numpy(all_cam_images)
        qpos_data = torch.from_numpy(qpos).float()  # (5,)
        action_data = torch.from_numpy(padded_action).float()   # (ep_len, 5)
        is_pad = torch.from_numpy(is_pad).bool()    # (ep_len,)

        # channel last
        image_data = torch.einsum('k h w c -> k c h w', image_data) # (1, 3, h, w)
        ...
        return image_data, qpos_data, action_data, is_pad, self.task_id
...
def get_norm_stats(dataset_dir, num_episodes):
    ...
    all_qpos_data = torch.stack(all_qpos_data)      # (n_eps, len, dim)
    all_action_data = torch.stack(all_action_data)  # (n_eps, len, dim)
...
def load_data(dataset_dir, num_episodes, camera_names, batch_size_train, batch_size_val, task_id):
    ...
    train_dataset = EpisodicDataset(train_indices, dataset_dir, camera_names, norm_stats, task_id)
    val_dataset = EpisodicDataset(val_indices, dataset_dir, camera_names, norm_stats, task_id)
...
def make_policy(policy_class, policy_config):
    ...
    elif policy_class == 'EMB':
        policy = EMBPolicy(policy_config)
    else:
